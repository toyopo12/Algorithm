...
int N, M;
Dinic network(40400, 0, 40040);
char board[110][110];
pair<int, int> good[110][110];
...
int main()
{
    ...
    for (int j = 1; j <= M; j++) {
        ...
        for (int i = 1; i <= N; i++) {
            if(board[i][j] == '2') mnum++;
            else good[i][j].second = mnum + 20000;
        }
    }
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= M; j++) {
            if(board[i][j] == '0') {
                ...
                network.addEdge(good[i][j].first + 10000, good[i][j].second, 10000, false);
                network.addEdge(good[i][j].second + 10000, good[i][j].first, 10000, false);
            }
        }
    }
    for(int i = 1; i <= 10000; i++) network.addEdge(i, i + 10000, 1, false);
    for(int i = 20001; i <= 30000; i++) network.addEdge(i, i + 10000, 1, false);
    for(int i = 30001; i <= 40000; i++) network.addEdge(i, 40040, 1, false);
    int ans = 0;
    while(network.bfs()) {
